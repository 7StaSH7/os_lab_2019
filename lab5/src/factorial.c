...
void Factorial(long long* res, int begin, int end, int module) {
    ...
    for (int i = begin; i < end; ++i) result = (result * i % module) % module;
    *res = result;
}
...
int main(int argc, char** argv) {
            ...
            case 0:
                switch (option_index) {
                    case 0:
                        number = atoi(optarg);
                        if (number < 0) {
                            printf("K must be positive number: %d\n", number);
                            return 1;
                        } break;
                    ...
                    case 2:
                        module = atoi(optarg);
                        if (module <= 1) {
                            printf("Module must be positive number and more than 1: %d\n", module);
                            return 1;
                        } break;
    ...
    for (int i = 0; i < pnum ; i++) {
        ...
        if (i < pnum - 1) FactArgs[i].end = FactArgs[i].begin + step; 
        else FactArgs[i].end = number + 1;              
    }
